...
describe('AuthService profile settings', () => {
  ...
  it('rejects an incorrect current password without changing data', async () => {
    ...
    await expect(serviceWith(prisma).changePassword('user-1', { currentPassword: 'wrong-password', newPassword: 'new-password' })).rejects.toBeInstanceOf(BadRequestException);
  });
});
