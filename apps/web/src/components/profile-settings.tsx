...
type Profile = ProfileValues & { loginId: string; roleName: string; portal: PortalType; status: string };
...
export function ProfileSettings({ initialProfile }: { initialProfile: Profile }) {
  ...
  return (
    <div className="grid max-w-5xl gap-5 xl:grid-cols-[minmax(0,1.1fr)_minmax(320px,0.9fr)]">
      <section className="rounded-xl border bg-white shadow-[0_1px_2px_rgba(23,32,51,0.025)]">
        ...
        <form className="grid gap-4 p-5 sm:grid-cols-2" onSubmit={profileForm.handleSubmit(saveProfile)} autoComplete="off" noValidate>
          ...
          <div className="flex flex-col-reverse gap-3 border-t pt-4 sm:col-span-2 sm:flex-row sm:items-center sm:justify-between">
            <div className="min-h-5 text-xs" aria-live="polite">
              {profileMessage && <span className="inline-flex items-center gap-1.5 text-[#067647]"><CheckCircle2 size={14} />{profileMessage}</span>}
      ...
      <section className="h-fit rounded-xl border bg-white shadow-[0_1px_2px_rgba(23,32,51,0.025)]">
        <div className="flex items-center gap-3 border-b px-5 py-4">
