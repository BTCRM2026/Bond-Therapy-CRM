    ...
    <>
      ...
      <section className="relative rounded-xl border bg-white shadow-[0_4px_14px_rgba(23,32,51,0.045)]">
        <div className="flex flex-col gap-3 border-b p-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <select value={roleFilter} onChange={(event) => setRoleFilter(event.target.value)} className="h-10 rounded-lg border bg-white px-3 text-[13px] text-foreground outline-none focus:border-brand focus:ring-2 focus:ring-brand/10" aria-label="Filter by role">
...
function StaffEditor({ mode, user, roles, managers, onClose, onSaved }: { mode: "create" | "edit"; user?: StaffUser; roles: Role[]; managers: StaffUser[]; onClose: () => void; onSaved: (message: string) => Promise<void> }) {
  ...
  const selectClass = "h-10 w-full rounded-lg border bg-white px-3 text-[13px] outline-none focus:border-brand focus:ring-2 focus:ring-brand/10";
