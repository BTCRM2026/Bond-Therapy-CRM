...
export async function POST(request: Request) {
  try {
    ...
    const upstream = await fetch(`${API}/attendance/punch-out`, { method: "POST", headers: { cookie: request.headers.get("cookie") ?? "", [PORTAL_HEADER]: portal }, cache: "no-store" });
    return new NextResponse(await upstream.text(), { status: upstream.status, headers: { "content-type": "application/json" } });
  } catch { return NextResponse.json({ message: "Attendance service is temporarily unavailable." }, { status: 503 }); }
}
