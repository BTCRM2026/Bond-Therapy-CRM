...
export async function GET(request: Request) {
  try {
    ...
    const upstream = await fetch(`${API}/clients/trainers`, { headers: { cookie: request.headers.get("cookie") ?? "", [PORTAL_HEADER]: portal }, cache: "no-store" });
    return new NextResponse(await upstream.text(), { status: upstream.status, headers: { "content-type": "application/json" } });
  } catch { return NextResponse.json({ message: "Trainer directory is temporarily unavailable." }, { status: 503 }); }
}
