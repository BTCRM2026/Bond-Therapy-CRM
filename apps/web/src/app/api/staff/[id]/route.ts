...
export async function PATCH(request: Request, { params }: RouteContext<"/api/staff/[id]">) {
  try {
    ...
    const upstream = await fetch(`${process.env.API_INTERNAL_URL ?? "http://localhost:3001"}/users/${encodeURIComponent(id)}`, {
      method: "PATCH",
    ...
    return NextResponse.json({ message: "Staff service is temporarily unavailable." }, { status: 503 });
  }
}

export async function DELETE(request: Request, { params }: RouteContext<"/api/staff/[id]">) {
  try {
    const portal = portalFromRequest(request);
    if (portal !== "ADMIN") return NextResponse.json({ message: "This resource is only available in the Admin portal." }, { status: 403 });
    const { id } = await params;
