...
import { SalesDashboardWidgets } from "@/components/sales-dashboard";
import { WarehouseDashboardWidgets } from "@/components/warehouse-dashboard";
...
export default async function DashboardPage() {
  ...
  if (session.portal === "STAFF") {
    const isSales = SALES_ROLE_KEYS.has(session.roles[0]?.key ?? "");
    const isWarehouse = WAREHOUSE_ROLE_KEYS.has(session.roles[0]?.key ?? "");
    return (
      <DashboardShell
      ...
      >
        <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div><h2 className="text-xl font-semibold tracking-[-0.02em] text-foreground">Welcome back, {session.name}</h2><p className="mt-1.5 text-sm text-muted">{isSales ? "Today's numbers and what needs your attention." : isWarehouse ? "What needs to ship and what needs restocking." : "Your secure account overview and assigned access."}</p></div>
          <span className="inline-flex w-fit items-center gap-2 rounded-lg border border-brand/15 bg-white px-3 py-2 text-xs font-semibold text-brand shadow-[0_1px_2px_rgba(15,23,42,0.03)]">
            <span className="size-2 rounded-full bg-success" />{roleName}
          </span>
        </div>

        {isSales ? <SalesDashboardWidgets /> : isWarehouse ? <WarehouseDashboardWidgets /> : (
          <div className="grid max-w-4xl gap-5 lg:grid-cols-[minmax(0,1.4fr)_minmax(260px,0.8fr)]">
            ...
            <section className="rounded-xl bg-brand p-5 text-white shadow-[0_10px_24px_rgba(37,99,235,0.14)]">
  ...
  return (
    ...
    >
      ...
      <section className="max-w-2xl rounded-xl border bg-white p-5 shadow-[0_3px_12px_rgba(15,23,42,0.04)] sm:p-6">
        <div className="flex items-start gap-4">
        ...
        <dl className="mt-5 grid gap-3 border-t pt-5 text-xs sm:grid-cols-3">
          ...
          <div><dt className="text-muted">Role</dt><dd className="mt-1 font-semibold text-foreground">{roleName}</dd></div>
          <div><dt className="text-muted">Session</dt><dd className="mt-1 inline-flex items-center gap-1.5 font-semibold text-success"><span className="size-1.5 rounded-full bg-success" />Protected</dd></div>
        </dl>
      </section>
    </DashboardShell>
  );
