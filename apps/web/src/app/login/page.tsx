  ...
  return (
    <main className="min-h-screen bg-background p-3 sm:p-5 lg:p-6">
      <section className="mx-auto grid min-h-[calc(100vh-24px)] max-w-[1440px] overflow-hidden rounded-xl border bg-white lg:min-h-[calc(100vh-48px)] lg:grid-cols-[1.05fr_0.95fr]">
        <div className="relative hidden overflow-hidden bg-[#26336f] p-12 text-white lg:flex lg:flex-col lg:justify-between xl:p-16">
          ...
          <div className="absolute -bottom-40 -right-28 size-[520px] rounded-full border border-white/15" />
          <div className="absolute -bottom-20 -right-48 size-[520px] rounded-full border border-white/10" />
          <div className="relative"><BrandMark inverse /></div>
          <div className="relative max-w-xl">
            <p className="mb-4 text-xs font-semibold uppercase tracking-[0.2em] text-white/60">One connected business workspace</p>
            ...
            <p className="mt-5 max-w-lg text-[15px] leading-7 text-white/70">Bond Therapy CRM brings sales, warehouse, accounts, people and distributor operations into one secure system.</p>
            <div className="mt-9 grid max-w-md gap-3">
              {capabilities.map(({ icon: Icon, label }) => (
                <div key={label} className="flex items-center gap-3 text-sm text-white/80">
                  <span className="grid size-8 place-items-center rounded-lg border border-white/15 bg-white/[0.07]"><Icon size={16} /></span>{label}
                </div>
              ))}
            </div>
          </div>
          <p className="relative text-xs text-white/45">Bond Therapy CRM · Authorized access only</p>
        </div>
